...
def main():
    ...
    while not flag_found and len(nodes) <= NUMNODES:
        ...
        # If all the nodes on the tree collide when connected to the random node:
        ...
        # If at least one does not collide, find the one that is nearest to the random node:
        else:
            print('At least one no coll')
        ...
        if len(nodes)%5 == 0:
            ...
            for p in nodes:
                if not checkIntersect(p, goal):
                    ...
                    flag_found  = True
                    print('Time it takes: ',sum(avg_time)/iter_num)
...
if __name__ == '__main__':
    ...
    print (" Vectorized RRT took ", eTime -sTime , "secs")
